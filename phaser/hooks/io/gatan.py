...
def load_gatan(args: None, props: LoadGatanProps) -> RawData:
    ...
    gatanFile = dm.file_reader(path, lazy=True)

    rawmetadata = gatanFile[0]['original_metadata']['ImageList']['TagGroup0']['ImageTags']


    if rawmetadata != None:  # load  metadata
        meta = GataMetadata.from_dm_metadata(rawmetadata)
